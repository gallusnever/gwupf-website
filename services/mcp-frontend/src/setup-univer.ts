...
function resolveSessionId() {
  ...
  const fromQuery = url.searchParams.get('univer_session_id')
  const fromEnv = importMeta?.env?.VITE_UNIVER_SESSION_ID
  const sessionId = fromQuery || fromEnv || 'default'
  if (!fromQuery && fromEnv) {
    url.searchParams.set('univer_session_id', sessionId)
